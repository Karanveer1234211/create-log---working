...
def _forward_outcomes(panel):
    """Vectorized next-open entry outcomes per symbol: entry_open, ret/mfe/mae for
    each horizon, and event flags over t+1..t+max(H). Adds columns in place."""
    p = panel.sort_values(["symbol", "date"]).copy()
    g = p.groupby("symbol", group_keys=False)
    p["entry_open"] = g["open"].shift(-1)
    maxh = max(HORIZONS)
    # avg volume (for spike), aligned to each future day later
    p["_avgvol"] = g["volume"].transform(lambda s: s.rolling(20, min_periods=5).mean().shift(1))
    for h in HORIZONS:
        exit_c = g["close"].shift(-h)
        highs = pd.concat([g["high"].shift(-k) for k in range(1, h + 1)], axis=1).max(axis=1)
        lows = pd.concat([g["low"].shift(-k) for k in range(1, h + 1)], axis=1).min(axis=1)
        p[f"ret_{h}"] = ((exit_c / p["entry_open"] - 1.0) * 100).round(3)
        p[f"mfe_{h}"] = ((highs / p["entry_open"] - 1.0) * 100).round(2)
        p[f"mae_{h}"] = ((lows / p["entry_open"] - 1.0) * 100).round(2)
    # events over t+1..t+maxh
    gap = move = spike = None
    for k in range(1, maxh + 1):
        pc = g["close"].shift(-(k - 1))
        gk = (g["open"].shift(-k) / pc - 1.0).abs()
        mk = (g["close"].shift(-k) / pc - 1.0).abs()
        sk = g["volume"].shift(-k) / g["_avgvol"].shift(-k)
        gap = gk if gap is None else np.maximum(gap, gk)
        move = mk if move is None else np.maximum(move, mk)
        spike = sk if spike is None else np.maximum(spike, sk)
    p["gap_max_pct"] = (gap * 100).round(2)
    p["move_max_pct"] = (move * 100).round(2)
    p["vol_spike_max"] = spike.round(2)
    p["ev_gap"] = (gap > GAP_THRESH).astype("Int64")
    p["ev_large_move"] = (move > MOVE_THRESH).astype("Int64")
    p["ev_vol_spike"] = (spike > VOL_MULT).astype("Int64")
    p["ev_any"] = ((p["ev_gap"] == 1) | (p["ev_large_move"] == 1) | (p["ev_vol_spike"] == 1)).astype("Int64")
    return p
...
def mode_backfill(a):
    feats = load_features(a.features)
    panel = load_panel(a.panel)
    test_start = pd.Timestamp(a.test_start).normalize()
    ...
    sc = sc.rename(columns={dcol: "date", scol: "symbol", pcol: "prob"})
    sc["date"] = _norm_day(sc["date"])
    sc = sc.dropna(subset=["prob"])
    sc["rank"] = sc.groupby("date")["prob"].rank(ascending=False, method="first")
    top = sc[sc["rank"] <= a.top_n][["symbol", "date", "prob", "rank"]].copy()
    print(f"[backfill] {len(top)} rows = top-{a.top_n} on {top['date'].nunique()} dates "
          f"({top['date'].min().date()} -> {top['date'].max().date()})")

    pout = _forward_outcomes(panel)
    feat_cols = [f for f in feats if f in pout.columns]
    out_cols = ["entry_open"] + [f"{m}_{h}" for m in ("ret", "mfe", "mae") for h in HORIZONS] \
        + ["ev_gap", "ev_large_move", "ev_vol_spike", "ev_any", "gap_max_pct", "move_max_pct", "vol_spike_max"]
    keep = ["symbol", "date", "close", "stock_regime"] + feat_cols + out_cols
    keep = [c for c in keep if c in pout.columns]
    merged = top.merge(pout[keep], on=["symbol", "date"], how="left")

    merged["is_oos"] = (merged["date"] >= test_start).astype(int)
    merged["status"] = np.where(merged["ret_5"].notna(), "RESOLVED", "PENDING")
    merged = merged.rename(columns={"close": "entry_ref", **{f: f"feat__{f}" for f in feat_cols}})
    merged["pred_date"] = merged["date"].dt.date
    merged["prob"] = merged["prob"].round(4)
    merged["rank"] = merged["rank"].astype(int)
    merged = merged.drop(columns=["date"])

    jrn = load_journal(a.journal)
    if len(jrn):
        merged = pd.concat([jrn, merged], ignore_index=True)
        merged = merged.drop_duplicates(subset=["pred_date", "symbol"], keep="last")
    save_journal(merged, a.journal)
    nres = (merged["status"] == "RESOLVED").sum()
    noos = ((merged["status"] == "RESOLVED") & (merged.get("is_oos", 0) == 1)).sum()
    print(f"[backfill] journal now {len(merged)} rows | resolved {nres} "
          f"(OOS {noos}, in-sample {nres - noos}) | features/row {len(feat_cols)}")
...
def _insample_summary(res_is):
    res_is = res_is.copy()
    res_is["ret_5"] = pd.to_numeric(res_is["ret_5"], errors="coerce")
    print("\n" + "=" * 64)
    print(f"IN-SAMPLE ROWS ({len(res_is)}) - CONTAMINATED, sanity-check only.")
    print("The model trained on these dates, so any 'edge' here is partly memorization.")
    print("Do NOT draw conclusions from this block; it's shown only for contrast.")
    print("=" * 64)
    res_is["bucket"] = (np.floor(pd.to_numeric(res_is["prob"], errors="coerce") / 0.05) * 0.05).round(2)
    for b, d in res_is.groupby("bucket"):
        print(f"  bucket {b:>5.2f}: n={len(d):>5}  win {(d['ret_5']>0).mean()*100:>4.0f}%  "
              f"mean ret_5 {d['ret_5'].mean():>+6.2f}")
...
def mode_analyze(a):
    ...
    res_is = pd.DataFrame()
    if "is_oos" in res.columns:
        res_is = res[pd.to_numeric(res["is_oos"], errors="coerce") == 0].copy()
        res = res[pd.to_numeric(res["is_oos"], errors="coerce") == 1].copy()
        print(f"[analyze] split on is_oos -> {len(res)} OUT-OF-SAMPLE rows (real evidence), "
              f"{len(res_is)} in-sample (contaminated, summarized separately at the end).")
    if len(res) < 10:
        print(f"[analyze] only {len(res)} OOS resolved rows. Accumulate more before trusting anything below.")
        if res.empty:
            if len(res_is):
                _insample_summary(res_is)
            return
    for c in ("prob", "ret_1", "ret_3", "ret_5", "mfe_5", "mae_5"):
        if c in res:
            res[c] = pd.to_numeric(res[c], errors="coerce")
    n = len(res)
    tag = "OUT-OF-SAMPLE " if len(res_is) or "is_oos" in jrn.columns else ""
    print(f"\n=== SAMPLE: {n} resolved {tag}predictions "
          f"({res['pred_date'].min()} -> {res['pred_date'].max()}) ===")
    print("    NOTE: everything below is a hypothesis on a finite sample. Confirm on a fresh slice before acting.\n")
...
def run_self_test():
    ...
    # --- backfill path: synth a scored panel (prob per row) and backfill top-5 ---
    scored = panel[["symbol", "timestamp", "close"]].copy()
    scored["prob"] = rng.uniform(0.4, 0.9, len(scored))
    scored.to_parquet(tmp / "scored.parquet")
    bjpath = str(tmp / "journal_bf.parquet")
    b = A(); b.scored = str(tmp / "scored.parquet"); b.panel = str(tmp / "panel.parquet")
    b.features = str(tmp / "feats.json"); b.journal = bjpath
    b.test_start = str(days[40].date()); b.top_n = 5
    print("\n--- backfill ---"); mode_backfill(b)
    jb = load_journal(bjpath)
    assert "is_oos" in jb.columns, "is_oos tag missing"
    assert (jb["is_oos"] == 1).any() and (jb["is_oos"] == 0).any(), "expected both OOS and in-sample"
    assert (jb["status"] == "RESOLVED").sum() > 50, "backfill should resolve most history"
    print("--- analyze (backfilled, OOS split) ---")
    ab = A(); ab.journal = bjpath; mode_analyze(ab)
    print("\nSELF-TEST OK")
    return 0
...
def main():
    ...
    ap.add_argument("--mode", choices=["record", "update", "analyze", "backfill"])
    ...
    ap.add_argument("--test-start", dest="test_start", default=None,
                    help="OOS boundary (from bigmove_meta.json), e.g. 2024-02-16")
    a = ap.parse_args()
    ...
    if a.mode == "record":
        ...
        if not (a.scored and a.panel and a.features and a.test_start):
            raise SystemExit("backfill needs --scored --panel --features --test-start --top-n")
        if a.top_n is None:
            raise SystemExit("backfill needs --top-n (e.g. 20)")
        mode_backfill(a)
    elif a.mode == "update":
        if not a.panel:
        ...
        raise SystemExit("pick --mode record|backfill|update|analyze (or --self-test)")
