...
FUZZY_DROP = ("symbol","rank","entry","close_d","close d","open_d",
              "movement","pct_movement",
              "ret_1","ret_3","ret1","ret3","ret_5","ret5",
              "mae_1","mae_3","mae1","mae3","mae_5",
              "mfe_1","mfe_3","mfe1","mfe3","mfe_5",
              "move_max","gap_max","max_pct","_max_pct",
              "ev_gap","ev_vol","ev_large","ev_any","gap?","volspike",
...
for c in df.columns:
    ...
    if s.notna().mean() < 0.80:      # not a usable numeric feature
        continue
    cl = c.lower()
    safe = cl.startswith(("feat__","x_")) or cl == "long_score"   # genuine model features, never drop
    if (not safe) and any(p in cl for p in FUZZY_DROP):
        continue
    df[c] = s
    feat_cols.append(c)
