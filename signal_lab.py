...
df["_d"]=pd.to_datetime(df[pick(df.columns,["timestamp","date","datetime","pred_date"])], errors="coerce").dt.normalize()
...
if (prob_c is None or oos_c is None) and args.picks:
    pk=load(args.picks); pk["_d"]=pd.to_datetime(pk[pick(pk.columns,["pred_date","date"])], errors="coerce").dt.normalize()
